...
function mk_error()
{
	echo -e "\033[47;31m>>>>>>>>>>>>>>ERROR: $*\033[0m"
}

function mk_warn()
{
	echo -e "\033[47;34m>>>>>>>>>>>>>>WARN: $*\033[0m"
}
...
LICHEE_BR2020_DIR=${LICHEE_TOP_DIR}/brandy/buildroot-2020.02.7

LICHEE_SATA_DIR=${LICHEE_TOP_DIR}/SATA
LICHEE_OUT_DIR=${LICHEE_TOP_DIR}/out
MKRULE_FILE=${LICHEE_TOOLS_DIR}/build/mkrule
LICHEE_ANDROID_DIR=${LICHEE_TOP_DIR}/android
LICHEE_SYSTEM_CONFIG_DIR=${LICHEE_TOOLS_DIR}/build/system

LICHEE_BR_RAMDISK_DIR=${LICHEE_OUT_DIR}/ramdisk
LICHEE_BR_RAMDISK_DIR_IMG=${LICHEE_BR_RAMDISK_DIR}/images

LICHEE_BR2020_ROOTFS_DIR=${LICHEE_OUT_DIR}/rootfs
LICHEE_BR2020_DIR_IMG=${LICHEE_BR2020_ROOTFS_DIR}/images
...
if [ ! -d ${LICHEE_KERN_DIR} -o \
	...
	mk_error "You are not at the top directory of sdk."
...
export LICHEE_ANDROID_DIR
export LICHEE_BR2020_DIR
export LICHEE_BR2020_DIR_IMG
export LICHEE_SYSTEM_CONFIG_DIR

export LICHEE_BR_RAMDISK_DIR
export LICHEE_BR_RAMDISK_DIR_IMG
...
function mk_select()
{
	...
	if [ "x${cfg_key}" == "xLICHEE_BOARD" ] ; then
		save_config "ANDROID_IMAGE_OUT" "${LICHEE_TOP_DIR}/android/out/target/product/tulip-${cfg_val}" $BUILD_CONFIG
	fi
}
...
function mkbr()
{
	...
	mk_bar "build buildroot OK."
}

function clbr()
{
	mk_bar "clean buildroot ..."

	local build_script="scripts/build.sh"
	(cd ${LICHEE_BR_DIR} && [ -x ${build_script} ] && ./${build_script} "clean")

	mk_bar "clean buildroot OK."
}
...
function prepare_dragonboard_toolchain()
{
	...
	mk_bar "Prepare dragonboard toolchain ..."
...
function mk_ramdisk()
{
	...
	if [ ${LICHEE_PLATFORM} = "vir" ]; then

		mk_bar "Build ramdisk rootfs ..."
		if [ "$1" = "f" ]; then
			rm -fr ${LICHEE_BR_RAMDISK_DIR}
		fi

		if [ ! -f ${LICHEE_BR_RAMDISK_DIR} ]; then
			mkdir -p ${LICHEE_BR_RAMDISK_DIR}
		fi

		make O=${LICHEE_BR_RAMDISK_DIR} -C ${LICHEE_BR2020_DIR} tulip_vir_ramdisk_defconfig
		make O=${LICHEE_BR_RAMDISK_DIR} -C ${LICHEE_BR2020_DIR}

		if [ -f ${LICHEE_BR_RAMDISK_DIR_IMG}/rootfs.cpio.gz ]; then
			cp -v ${LICHEE_BR_RAMDISK_DIR_IMG}/rootfs.cpio.gz ${LICHEE_KERN_DIR}/ramdisk/${LICHEE_PLATFORM}
		fi
...
function mkkernel()
{
	...
	local build_script="scripts/buildKernel.sh"
...
function clkernel()
{
	...
	mk_bar "clean kernel OK."
}

function cldragonboard()
{
	mk_bar "clean dragonboard ..."
	...
	mk_bar "clean dragonboard OK."
}

function mkboot()
{
	mk_bar "build boot ..."
	mk_bar "build boot OK."
}
...
function mk_tinyandroid()
{
	...
	mk_bar "Build tinyandroid rootfs ..."
	if [ "$1" = "f" ]; then
		rm -fr ${ROOTFS}
	fi
...
function mk_defroot()
{
	...
	if [ ! -f ${ROOTFS} ]; then
		...
		if [ -n "`echo $LICHEE_KERN_VER | grep "linux-4.[49]"`" ]; then
			#fakeroot tar -zxvf ${LICHEE_TOOLS_DIR}/build/rootfs_tar/alpine-minirootfs-3.12.0-${LICHEE_ARCH}.tar.gz -C ${ROOTFS}
			fakeroot tar -zxvf ${LICHEE_TOOLS_DIR}/build/rootfs_tar/alpine-buildroot-${LICHEE_ARCH}.tar.gz -C ${ROOTFS}
	...
	mkdir -p ${ROOTFS}/lib/modules
	#cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* ${ROOTFS}/lib/modules/
	...
	# (cd ${ROOTFS};  sed -i 's/::sysinit:\/etc\/init.d\/rcS/#/g' etc/inittab) 
	# (cd ${ROOTFS};  sed -i 's/tty1::respawn/#tty1::respawn/g' etc/inittab)
	# (cd ${ROOTFS};  sed -i 's/tty2::respawn/#tty2::respawn/g' etc/inittab)
	# (cd ${ROOTFS};  sed -i 's/tty3::respawn/#tty3::respawn/g' etc/inittab)
	# (cd ${ROOTFS};  sed -i 's/tty4::respawn/#tty4::respawn/g' etc/inittab)
	# (cd ${ROOTFS};  sed -i 's/tty5::respawn/#tty5::respawn/g' etc/inittab)
	# (cd ${ROOTFS};  sed -i 's/tty6::respawn/#tty6::respawn/g' etc/inittab)

	# (cd ${ROOTFS};  sed -i 's/#ttyS0/ttyS0/g' etc/inittab)
...
function mk_br2020_rootfs()
{
	...
	make O=${LICHEE_BR2020_ROOTFS_DIR} -C ${LICHEE_BR2020_DIR} tulip_vir_defconfig
	make O=${LICHEE_BR2020_ROOTFS_DIR} -C ${LICHEE_BR2020_DIR}

	if [ ! -f ${ROOTFS} ]; then
		mkdir -p ${ROOTFS}
	fi

	if [ -f ${LICHEE_BR2020_DIR_IMG}/xen ]; then
		cp -v ${LICHEE_BR2020_DIR_IMG}/xen ${LICHEE_PLAT_OUT}
	fi

	if [ -f ${LICHEE_BR2020_DIR_IMG}/rootfs.tar.gz ]; then
		fakeroot tar -zxf ${LICHEE_BR2020_DIR_IMG}/rootfs.tar.gz -C ${ROOTFS}

		mkdir -p ${ROOTFS}/lib/modules
		cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* ${ROOTFS}/lib/modules/
		cp -v ${LICHEE_KERN_DIR}/output/bImage ${LICHEE_PLAT_OUT}/Image
		cp -v ${LICHEE_SYSTEM_CONFIG_DIR}/monitorConfig.its ${LICHEE_PLAT_OUT}
		cp -v ${LICHEE_SYSTEM_CONFIG_DIR}/mkBoot.sh ${LICHEE_PLAT_OUT}
		...
		(cd ${ROOTFS}; ln -fs sbin/openrc-init init)
		# (cd ${ROOTFS}; sed -i 's/root:x:/root::/g' etc/passwd)
		...
		# 2097152 2G
		# 3145728 3G
		fakeroot mke2img -d ${ROOTFS} -G 4 -R 1 -B 4194304 -I 1048576 -o ${LICHEE_PLAT_OUT}/rootfs.ext4
cat  > ${LICHEE_PLAT_OUT}/.rootfs << EOF
chown -h -R 0:0 ${ROOTFS}
${LICHEE_TOOLS_DIR}/build/bin/mksquashfs \
${ROOTFS} ${LICHEE_PLAT_OUT}/rootfs.squashfs -root-owned -no-progress -comp xz -noappend
...
function mkrootfs()
{
	...
	elif [ ${LICHEE_PLATFORM} = "android" ]; then
		local build_script="build.sh"
		mk_bar "make andorid system rootfs"

		(cd ${LICHEE_ANDROID_DIR} && [ -x ${build_script} ] && ./${build_script})

	elif [ ${LICHEE_PLATFORM} = "vir" ]; then
		mk_bar "make vir system rootfs"
...
function mkclean()
{
	...
	mk_bar "clean product output in ${LICHEE_PLAT_OUT} ..."
	cd ${LICHEE_PLAT_OUT}
...
function mkdistclean()
{
	...
	cldragonboard
	mk_bar "clean entires output dir ..."
	rm -rf ${LICHEE_OUT_DIR}
}

function mkpack()
{
	mk_bar "packing firmware ..."
