...
{
    echo "######################################################################"
    echo -e "\033[47;30mKERNEL: $*\033[0m"
    echo "######################################################################"
}
...
build_kernel()
{
    ...
    kernel_bar "Copy rootfs.cpio.gz(RAMDISK) for ${ARCH} of ${LICHEE_PLATFORM} "
    if [ "${ARCH}" = "arm" ]; then
        cp -f ./ramdisk/${LICHEE_PLATFORM}/rootfs_32bit.cpio.gz output/rootfs.cpio.gz
 else
        cp -f ./ramdisk/${LICHEE_PLATFORM}/rootfs.cpio.gz output/rootfs.cpio.gz
    fi
...
regen_rootfs_cpio()
{
    kernel_bar "regenerate rootfs cpio"

    cd ${LICHEE_KDIR}/output
    if [ -x "../scripts/buildRootfs.sh" ]; then
        ../scripts/buildRootfs.sh e ./rootfs.cpio.gz > /dev/null
    else
        echo "No such file: scripts/buildRootfs.sh"
        exit 1
    fi
    ...
    rm -f ./rootfs.cpio.gz
    ../scripts/buildRootfs.sh c ./rootfs.cpio.gz > /dev/null
    rm -rf skel
    cd - > /dev/null
}
...
clean_kernel()
{
    ...
    kernel_bar "Cleaning kernel, arg: $clarg ..."
...
case "$1" in
    ...
    *)
        ...
        build_ramfs
        gen_output
        kernel_bar "${LICHEE_CHIP} compile Kernel successful"
        ;;
esac
